...
export default function ActorForm({title,btnTitle,onSubmit,busy,initialState}) {
  ...
  return (
        ...
        <form className='dark:bg-primary bg-white p-3 w-[35rem] rounded '
         onSubmit={handleSubmit}> 
        <div className="flex justify-between items-center mb-3">
            <h1 className="font-semibold text-xl dark:text-white text-primary">{title}</h1>
            <button className='h-8 w-24 bg-primary text-white dark:bg-white dark:text-primary
             hover:opacity-80 transition rounded flex items-center justify-center' type='submit'>
            {busy?<ImSpinner3 className='animate-spin'/>: btnTitle}</button>
        </div>
        ...
        <div className='flex space-x-2'>
            ...
            <div className="flex-grow flex flex-col space-y-2">
                <input name='name'
                value={name}
                onChange={handleChange} placeholder='Enter name' type='text' className={commonInputClasses+" border-b-2"}/>
                <textarea name='about'
                value={about}
                 onChange={handleChange} placeholder='About'  
                 className={commonInputClasses+" border-b-2 resize-none h-full"}></textarea>
            </div>
        </div>
        <div className="mt-3">
