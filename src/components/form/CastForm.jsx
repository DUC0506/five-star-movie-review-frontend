...
export default function CastForm({onSubmit}) {
  ...
  return (
    <div className='flex items-center space-x-2'>
        <input type="checkbox" name='leadActor' className='w-4 h-4' checked={leadActor}
        onChange={handleOnChange}
        title='Set as lead Actor' />

        <LiveSearch placeholder='Search Profile ' value={profile.name}
        results={profiles}
        ...
        placeholder='Role as' value={roleAs} name='roleAs'
        onChange={handleOnChange} />
        </div>
        
        <button
        onClick={handleSubmit}
        type='button' className='bg-secondary dark:bg-white dark:text-primary text-white px-1 rounded'>Add</button>
    </div>
  )
}
