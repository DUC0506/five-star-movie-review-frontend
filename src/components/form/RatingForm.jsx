  ...
  return (
    <div>
        <div className="p-5 dark:bg-primary bg-white rounded space-y-3">
            ...
            <textarea value={content} onChange={handleOnChange} className='w-full h-24 border-2 p-2 dark:text-white text-primary rounded outline-none
             bg-transparent resize-none '></textarea>
...
const StarsOutLined=({ratings,onMouseEnter})=>{
    return ratings.map((_,index)=>{
        return <AiOutlineStar onMouseEnter={()=>onMouseEnter(index)} className='cursor-pointer' key={index} size={24}/>  
    })
}
const StarsFilled=({ratings,onMouseEnter})=>{
    return ratings.map((_,index)=>{
        return <AiFillStar onMouseEnter={()=>onMouseEnter(index)} className='cursor-pointer' key={index} size={24}/>  
    })
}